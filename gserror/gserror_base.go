...
func CheckErrorEx(err error, inlogger *log.Logger) {
	if err != nil {
		...
		refuncName := runtime.FuncForPC(pc).Name()
		funcName := refuncName[strings.LastIndex(refuncName, "/")+1:]
		if inlogger != nil {
			tmp := make([]byte, G_gsErrorStackBufSize)
			nlen := runtime.Stack(tmp, false)
			inlogger.Output(3, fmt.Sprintf("[%s]: Error: %s\nstack: %s\n", funcName, err.Error(), string(tmp[:nlen])))
		} else {
			tmp := make([]byte, G_gsErrorStackBufSize)
			nlen := runtime.Stack(tmp, false)
...
func CheckErrorEx_info(err error, inlogger *log.Logger) {
	if err != nil {
		...
		refuncName := runtime.FuncForPC(pc).Name()
		funcName := refuncName[strings.LastIndex(refuncName, "/")+1:]
		inlogger.Output(3, fmt.Sprintf("[%s]: Info: %s\n", funcName, err.Error()))
	}
}
